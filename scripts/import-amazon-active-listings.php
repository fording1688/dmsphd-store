...
if ($report === '' || !is_readable($report)) {
    fwrite(STDERR, "Usage: php scripts/import-amazon-active-listings.php /path/to/report.txt\n");
    exit(1);
}
...
while (($row = fgetcsv($handle, 0, "\t")) !== false) {
    ...
    $product->set_stock_status($quantity !== '' && (int) $quantity <= 0 ? 'outofstock' : 'instock');
...
echo "Created/updated products: " . ($created + $updated) . "\n";
